// Ported from Snapduka/src/lib/billing/subscriptions.ts — pure state logic.
...
export function canTransitionSubscription(from: SubscriptionState, to: SubscriptionState) {
  return transitions[from].includes(to);
}

export function effectiveSubscriptionState(
  subscription: { state: SubscriptionState; graceEndsAt: string | null },
  now = new Date(),
): SubscriptionState {
  if (subscription.state === "past_due" && subscription.graceEndsAt) {
