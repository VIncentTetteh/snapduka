...
export function ProductGrid({ slug, products }: { slug: string; products: Product[] }) {
  if (!products.length) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      {products.map((product) => {
        ...
        return (
          <Link className="grid min-w-0 gap-2 rounded-2xl border border-stone-200 bg-white p-3 text-stone-950 no-underline shadow-sm" href={`/${slug}/products/${product.id}`} key={product.id}>
            ...
            <strong>{product.currency} {(product.price_minor / 100).toFixed(2)}</strong>
