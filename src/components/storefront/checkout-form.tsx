...
export function CheckoutForm({
  shopId, country, product, methods, campaignToken,
}: {
  shopId: string;
  country: CountryCode;
  ...
  return (
    <form className="grid gap-3" onSubmit={async (event) => {
      ...
      try {
        const response = await fetch("/api/checkout/orders", {
          ...
          body: JSON.stringify({
            ...
            promotionCode: values.get("promotionCode"), campaignToken,
            buyer: {
      ...
      <section className="rounded-2xl border border-stone-300 bg-white p-4">
        ...
        <p>{product.currency} {product.currency === "XOF" ? product.price_minor : (product.price_minor / 100).toFixed(2)}</p>
      ...
      <section className="grid gap-3 rounded-2xl border border-stone-300 bg-white p-4">
        ...
        <input className={input} autoComplete="tel" name="phone" placeholder={country === "GH" ? "024 123 4567" : country === "NG" ? "0801 234 5678" : "07 08 09 10 11"} required />
        ...
        <input className={input} autoComplete="address-level1" name="region" placeholder={country === "NG" ? "State" : "Region"} />
      </section>
      <section className="grid gap-3 rounded-2xl border border-stone-300 bg-white p-4">
        ...
        <input className={input} name="promotionCode" placeholder="Promotion code (optional)" />
        <select className={input} name="paymentMethod">{country !== "CI" && <option value="paystack">Pay online with Paystack</option>}<option value="cash_on_delivery">Cash on delivery</option><option value="seller_arranged">Arrange payment with seller</option></select>
