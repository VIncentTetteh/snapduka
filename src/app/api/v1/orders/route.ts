...
/**
 * GET /api/v1/orders — the API key's seller's orders, oldest first. `event_version` is the value to send as `expectedVersion` to POST /api/v1/fulfillment.
 *
 ...
 */
const COLUMNS = "id,public_reference,status,event_version,payment_status,fulfillment_status,refund_status,dispute_status,currency,total_minor,created_at";

export async function GET(request: Request): Promise<NextResponse> {
  const auth = await authenticateApi(request, "orders:read");
