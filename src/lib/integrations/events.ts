...
export async function enqueueIntegrationEvent(input: {
  ...
  if (hooks?.length) {
    await admin.from("webhook_deliveries").upsert(hooks.map((hook) => ({ event_id: input.eventId, payload, seller_account_id: input.sellerAccountId, webhook_id: hook.id })), { onConflict: "webhook_id,event_id", ignoreDuplicates: true });
  }
  if (rules?.length) {
    await admin.from("automation_runs").upsert(rules.map((rule) => ({ depth: input.depth ?? 0, event_id: input.eventId, result: { event: payload }, rule_id: rule.id, seller_account_id: input.sellerAccountId, state: "pending" })), { onConflict: "rule_id,event_id", ignoreDuplicates: true });
  }
}
