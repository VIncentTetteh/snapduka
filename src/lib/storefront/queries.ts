...
export async function getPublicShop(slug: string) {
  const { data, error } = await publicClient()
    ...
    .select("id, seller_account_id, slug, slug_code, display_name, country, currency, published_at, verified_at, fulfillment_methods(type,active), shop_branding(accent_color,surface_color,font_family,logo_path,banner_path,hide_snapduka_branding,whatsapp_number)")
    .eq("slug", slug)
    .eq("status", "published")
    .maybeSingle();
