...
-- anon lost this in 202608010065: calling it directly bypassed the checkout
-- route's rate limiting. Orders are created server-side with service_role.
select is(has_function_privilege('anon','public.create_guest_order_growth(uuid,uuid,jsonb,jsonb,text,text,text,text,uuid)','EXECUTE'),false,'buyers cannot call the order RPC directly');
